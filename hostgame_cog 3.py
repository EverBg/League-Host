...
async def get_thread(interaction: discord.Interaction, game: dict):
    thread = interaction.guild.get_thread(game.get("thread"))
    if thread is None:
        thread = await interaction.guild.fetch_channel(game.get("thread"))
    return thread
...
def get_player_tier(guild: discord.Guild, player_id: int):
    ...
    for tier in TIER_ROLES:
        if any(tier.lower() in role.name.lower() for role in member.roles):
            return tier
...
def get_ping_role(guild: discord.Guild):
    ...
    for role in guild.roles:
        normalized_role = role.name.lower().replace(" ", "")
        if role.name.lower() == target.lower() or normalized_role == normalized_target:
            return role
        if target.lower() in role.name.lower():
            return role
...
class HostGame(commands.Cog):
    ...
    async def createteams(self, interaction: discord.Interaction):
        ...
        if not any(p["id"] == game.get("host_id") for p in players):
            host_member = interaction.guild.get_member(game.get("host_id"))
            host_name = host_member.display_name if host_member else game.get("host_name")
            players.append({"id": game.get("host_id"), "display_name": host_name})
        ...
        by_tier = {}
        for p in players:
            tier = get_player_tier(interaction.guild, p.get("id"))
